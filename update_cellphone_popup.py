...
class UpdateCellphonePopup(tk.Frame):
    ...
    def __init__(self, parent, close_callback, product):
        ...
        tk.Label(self, text="Camera:").grid(row=8, column=1)
        self._camera = tk.Entry(self)
        self._camera.grid(row=8, column=2)
        self._camera.insert(0, product["camera"])
        tk.Label(self, text="Security:").grid(row=9, column=1)
        tk.Label(self, text=product["security"]).grid(row=9, column=2)
        tk.Label(self, text="Screen Body Ratio:").grid(row=10, column=1)
        tk.Label(self, text=product["screen_body_ratio"]).grid(row=10, column=2)
